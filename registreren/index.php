...
if (isset($_POST["knop_registreer"])) {
    ...
    // Inhoud van de schermvelden overbrengen naar lokale variabelen
    ...
    $adminkey = 0; // Standaard geen admin

    // Debugging: Toon alle ingevoerde gegevens vóór de query
    echo "<pre>";
    var_dump([
        'naam' => $naam, 
        'voornaam' => $voornaam, 
    ...
    // SQL-query
    $qryInsertKlant = "INSERT INTO tblKlanten 
        (naam, voornaam, geboortedatum, geslacht, straat, huisnummer, busnummer, postcode, stad, land, telefoonnummer, email, wachtwoord, adminkey)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

    // Prepared statement
    if ($stmInsertKlant = mysqli_prepare($dbconn, $qryInsertKlant)) {
        // Waardes binden aan de query
        ...
        // Query uitvoeren
        if (mysqli_stmt_execute($stmInsertKlant)) {
            echo "Stap 4: Registratie succesvol!<br>";
            header("Location: ../aanmelden/index.php");
            exit();
        } else {
            die("Fout bij registratie: " . mysqli_stmt_error($stmInsertKlant));
        }

        // Statement sluiten
        mysqli_stmt_close($stmInsertKlant);
    } else {
...
<head>
    ...
    <script>
        tailwind.config = {
            theme: {
                extend: {
                    colors: {
                        primary: '#C8102E',    // Refined red for elegance
                        secondary: '#0A6C93',  // Deep teal for sophistication
                        accent: '#F4C430',     // Warm gold for premium feel
                        dark: '#12122A',       // Darker navy for depth
                    }
                }
            }
        };
    ...
    <style>
        ...
        .glow:focus { box-shadow: 0 0 0 3px rgba(200, 16, 46, 0.5); }
        /* Button hover lift */
        .hover-lift { transition: transform 0.3s ease, box-shadow 0.3s ease; }
        .hover-lift:hover { transform: translateY(-3px); box-shadow: 0 6px 12px rgba(0, 0, 0, 0.3); }
        /* Progress bar line */
        ...
        .progress-line::before {
            ...
            width: 60%;
            height: 2px;
            background: linear-gradient(to right, #C8102E 50%, #4B5563 50%);
            z-index: -1;
        }
        ...
        select option { background-color: #12122A !important; color: white !important; }
        select option:hover { background-color: #C8102E !important; color: white !important; }
        select:focus { outline: none; box-shadow: 0 0 0 3px rgba(200, 16, 46, 0.5); }
        /* Scrollbar for popups */
        .popup-content::-webkit-scrollbar { width: 8px; }
        .popup-content::-webkit-scrollbar-track { background: #12122A; }
        .popup-content::-webkit-scrollbar-thumb { background: #C8102E; border-radius: 4px; }
        .popup-content::-webkit-scrollbar-thumb:hover { background: #a00d26; }
    </style>
</head>
<body class="bg-gradient-to-b from-dark to-[#0A0A1F] text-white font-['Inter']">
    <!-- Navbar -->
    ...
    <section class="min-h-screen flex items-center justify-center pt-20 pb-20">
        <div class="container mx-auto px-4 sm:px-6 lg:px-8">
            <div class="max-w-xl mx-auto bg-[rgba(255,255,255,0.05)] rounded-2xl p-8 sm:p-10 backdrop-blur-lg shadow-2xl fade-in">
                <h2 class="text-3xl sm:text-4xl font-bold text-center mb-6 font-['Playfair_Display']">Registreren bij <span class="text-primary">MAQUA</span></h2>
                <p class="text-gray-300 text-center mb-8 text-sm sm:text-base">Maak een account aan om deel uit te maken van onze gemeenschap.</p>
                    ...
                    <div class="form-step" data-step="1">
                        <div class="space-y-6">
                            <div class="form-group">
                                ...
                                <input type="text" id="voornaam" name="voornaam" class="w-full bg-[rgba(255,255,255,0.1)] text-white border-none rounded-lg py-3 px-4 focus:outline-none glow transition-all" placeholder="Voornaam">
                                <span class="error-text text-red-500 text-xs hidden mt-1">Voer een geldige voornaam in</span>
                            </div>
                            <div class="form-group">
                                <label for="naam" class="block text-sm font-medium mb-2">Achternaam</label>
                                <input type="text" id="naam" name="naam" class="w-full bg-[rgba(255,255,255,0.1)] text-white border-none rounded-lg py-3 px-4 focus:outline-none glow transition-all" placeholder="Achternaam">
                                <span class="error-text text-red-500 text-xs hidden mt-1">Voer een geldige achternaam in</span>
                            </div>
                            <div class="form-group">
                                <label for="land" class="block text-sm font-medium mb-2">Land</label>
                                <button type="button" id="land-btn" class="w-full bg-[rgba(255,255,255,0.1)] text-white border-none rounded-lg py-3 px-4 text-left focus:outline-none glow transition-all flex items-center justify-between hover:bg-[rgba(255,255,255,0.15)]">
                                    <span id="land-selected">Selecteer een land</span>
                                    <i class="fas fa-chevron-down"></i>
                                </button>
                                <input type="hidden" id="land" name="land">
                                <span class="error-text text-red-500 text-xs hidden mt-1">Selecteer een geldig land</span>
                            </div>
                            <div class="form-group">
                                <label for="geboortedatum" class="block text-sm font-medium mb-2">Geboortedatum</label>
                                <button type="button" id="date-btn" class="w-full bg-[rgba(255,255,255,0.1)] text-white border-none rounded-lg py-3 px-4 text-left focus:outline-none glow transition-all flex items-center justify-between hover:bg-[rgba(255,255,255,0.15)]">
                                    <span id="date-selected">Kies je geboortedatum</span>
                                    <i class="fas fa-calendar-alt"></i>
                                </button>
                                <input type="hidden" id="geboortedatum" name="geboortedatum">
                                <span class="error-text text-red-500 text-xs hidden mt-1">Voer een geldige geboortedatum in</span>
                            </div>
                            <div class="form-group">
                                <label class="block text-sm font-medium mb-2">Geslacht</label>
                                <div class="flex space-x-4">
                                    <button type="button" class="gender-btn flex-1 py-3 px-4 bg-[rgba(255,255,255,0.1)] rounded-lg hover:bg-[rgba(255,255,255,0.15)] transition-all hover-lift" data-value="man">
                                        <i class="fas fa-mars mr-2"></i>Man
                                    </button>
                                    <button type="button" class="gender-btn flex-1 py-3 px-4 bg-[rgba(255,255,255,0.1)] rounded-lg hover:bg-[rgba(255,255,255,0.15)] transition-all hover-lift" data-value="vrouw">
                                        <i class="fas fa-venus mr-2"></i>Vrouw
                                    </button>
                                </div>
                                <input type="hidden" id="geslacht" name="geslacht">
                                <span class="error-text text-red-500 text-xs hidden mt-1">Selecteer een geslacht</span>
                            </div>
                        </div>
                        <button type="button" class="w-full bg-primary text-white py-3 rounded-lg font-semibold hover:bg-[#a00d26] transition-colors hover-lift mt-6" onclick="validateStep1()">Volgende</button>
                    </div>
                    ...
                    <div class="form-step hidden" data-step="2">
                        <div class="space-y-6">
                            <div class="form-group">
                                ...
                                <div class="flex">
                                    <button type="button" id="phone-prefix-btn" class="bg-[rgba(255,255,255,0.1)] text-white border-none rounded-l-lg py-3 px-4 focus:outline-none glow transition-all flex items-center hover:bg-[rgba(255,255,255,0.15)]">
                                    ...
                                    <input type="tel" id="telefoonnummer" name="telefoonnummer" class="flex-1 bg-[rgba(255,255,255,0.1)] text-white border-none rounded-r-lg py-3 px-4 focus:outline-none glow transition-all" placeholder="123 456 789">
                                </div>
                                <input type="hidden" id="phone-prefix" name="phone-prefix" value="+32">
                                <span class="error-text text-red-500 text-xs hidden mt-1">Voer een geldig telefoonnummer in</span>
                            </div>
                            <div class="form-group">
                                <label for="straat" class="block text-sm font-medium mb-2">Straat</label>
                                <input type="text" id="straat" name="straat" class="w-full bg-[rgba(255,255,255,0.1)] text-white border-none rounded-lg py-3 px-4 focus:outline-none glow transition-all" placeholder="Straatnaam">
                                <span class="error-text text-red-500 text-xs hidden mt-1">Voer een geldige straatnaam in</span>
                            </div>
                            <div class="flex space-x-4">
                                <div class="form-group flex-1">
                                    <label for="huisnummer" class="block text-sm font-medium mb-2">Huisnummer</label>
                                    <input type="text" id="huisnummer" name="huisnummer" class="w-full bg-[rgba(255,255,255,0.1)] text-white border-none rounded-lg py-3 px-4 focus:outline-none glow transition-all" placeholder="123">
                                    <span class="error-text text-red-500 text-xs hidden mt-1"></span>
                                </div>
                                <div class="form-group flex-1">
                                    <label for="busnummer" class="block text-sm font-medium mb-2">Busnummer</label>
                                    <input type="text" id="busnummer" name="busnummer" class="w-full bg-[rgba(255,255,255,0.1)] text-white border-none rounded-lg py-3 px-4 focus:outline-none glow transition-all" placeholder="Optioneel">
                                    <span class="error-text text-red-500 text-xs hidden mt-1"></span>
                                </div>
                            </div>
                            <div class="flex space-x-4">
                                <div class="form-group flex-1">
                                    ...
                                    <input type="text" id="postcode" name="postcode" class="w-full bg-[rgba(255,255,255,0.1)] text-white border-none rounded-lg py-3 px-4 focus:outline-none glow transition-all" placeholder="3000">
                                    <span class="error-text text-red-500 text-xs hidden mt-1">Voer een geldige postcode in</span>
                                </div>
                                <div class="form-group flex-1">
                                    <label for="stad" class="block text-sm font-medium mb-2">Stad</label>
                                    <input type="text" id="stad" name="stad" class="w-full bg-[rgba(255,255,255,0.1)] text-white border-none rounded-lg py-3 px-4 focus:outline-none glow transition-all" placeholder="Antwerpia">
                                    <span class="error-text text-red-500 text-xs hidden mt-1">Voer een geldige stad in</span>
                                </div>
                            </div>
                        </div>
                        <div class="flex space-x-4 mt-6">
                            <button type="button" class="w-full bg-gray-600 text-white py-3 rounded-lg font-semibold hover:bg-gray-700 transition-colors hover-lift prev-step">Vorige</button>
                            <button type="button" class="w-full bg-primary text-white py-3 rounded-lg font-semibold hover:bg-[#a00d26] transition-colors hover-lift next-step" onclick="validateStep2()">Volgende</button>
                        </div>
                    </div>
                    ...
                    <div class="form-step hidden" data-step="3">
                        <div class="space-y-6">
                            <div class="form-group">
                                ...
                                <div class="relative">
                                    <input type="email" id="email" name="email" class="w-full bg-[rgba(255,255,255,0.1)] text-white border-none rounded-lg py-3 px-4 focus:outline-none glow transition-all" placeholder="jouw@email.com">
                                    <i class="fas fa-envelope absolute right-4 top-1/2 transform -translate-y-1/2 text-gray-400"></i>
                                </div>
                                <span class="error-text text-red-500 text-xs hidden mt-1">Voer een geldig e-mailadres in</span>
                            </div>
                            <div class="form-group">
                                <label for="wachtwoord" class="block text-sm font-medium mb-2">Wachtwoord</label>
                                <div class="relative">
                                    <input type="password" id="wachtwoord" name="wachtwoord" class="w-full bg-[rgba(255,255,255,0.1)] text-white border-none rounded-lg py-3 px-4 focus:outline-none glow transition-all" placeholder="••••••••">
                                ...
                                <span class="error-text text-red-500 text-xs hidden mt-1">Voer een geldig wachtwoord in</span>
                            </div>
                            <div class="form-group">
                                <label for="wachtwoord-confirm" class="block text-sm font-medium mb-2">Bevestig wachtwoord</label>
                                <div class="relative">
                                    <input type="password" id="wachtwoord-confirm" name="wachtwoord-confirm" class="w-full bg-[rgba(255,255,255,0.1)] text-white border-none rounded-lg py-3 px-4 focus:outline-none glow transition-all" placeholder="••••••••">
                                    <button type="button" class="toggle-password absolute right-4 top-1/2 transform -translate-y-1/2 text-gray-400 hover:text-primary">
                                        <i class="far fa-eye"></i>
                                    </button>
                                </div>
                                <span class="error-text text-red-500 text-xs hidden mt-1">Wachtwoorden moeten overeenkomen</span>
                            </div>
                            <div class="form-group">
                                <label class="flex items-center space-x-2">
                                    <input type="checkbox" id="reg-terms" name="terms" class="form-checkbox text-primary h-5 w-5 rounded focus:ring-primary">
                                ...
                                <span class="error-text text-red-500 text-xs hidden mt-1">Accepteer de voorwaarden</span>
                            </div>
                        </div>
                        <div class="flex space-x-4 mt-6">
                            <button type="button" class="w-full bg-gray-600 text-white py-3 rounded-lg font-semibold hover:bg-gray-700 transition-colors hover-lift prev-step">Vorige</button>
                            <button type="submit" name="knop_registreer" class="w-full bg-primary text-white py-3 rounded-lg font-semibold hover:bg-[#a00d26] transition-colors hover-lift flex items-center justify-center space-x-2">
                ...
                <div id="land-popup" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center hidden z-50">
                    <div class="bg-dark rounded-xl p-6 w-full max-w-md max-h-[80vh] popup-content overflow-y-auto shadow-xl">
                        <h3 class="text-lg font-bold mb-4 font-['Playfair_Display']">Kies een land</h3>
                        <div class="space-y-2">
                            <button class="country-option w-full text-left py-3 px-4 bg-[rgba(255,255,255,0.1)] rounded-lg hover:bg-[rgba(255,255,255,0.15)] transition-all hover-lift" data-value="BE">
                                <img src="https://flagcdn.com/24x18/be.png" alt="België" class="inline-block mr-2 w-6 h-4"> België
                            </button>
                            <button class="country-option w-full text-left py-3 px-4 bg-[rgba(255,255,255,0.1)] rounded-lg hover:bg-[rgba(255,255,255,0.15)] transition-all hover-lift" data-value="NL">
                                <img src="https://flagcdn.com/24x18/nl.png" alt="Nederland" class="inline-block mr-2 w-6 h-4"> Nederland
                            </button>
                            <!-- Voeg meer landen toe met hun vlaggen -->
                        </div>
                        <button id="close-land-popup" class="mt-4 w-full bg-gray-600 text-white py-2 rounded-lg hover:bg-gray-700 transition-all hover-lift">Sluiten</button>
                    </div>
                </div>
                ...
                <div id="phone-prefix-popup" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center hidden z-50">
                    <div class="bg-dark rounded-xl p-6 w-full max-w-md max-h-[80vh] popup-content overflow-y-auto shadow-xl">
                        <h3 class="text-lg font-bold mb-4 font-['Playfair_Display']">Kies een landcode</h3>
                        <div class="space-y-2">
                            <button class="prefix-option w-full text-left py-3 px-4 bg-[rgba(255,255,255,0.1)] rounded-lg hover:bg-[rgba(255,255,255,0.15)] transition-all hover-lift" data-prefix="+32" data-country="BE">
                                <img src="https://flagcdn.com/24x18/be.png" alt="België" class="inline-block mr-2 w-6 h-4"> België (+32)
                            </button>
                            <button class="prefix-option w-full text-left py-3 px-4 bg-[rgba(255,255,255,0.1)] rounded-lg hover:bg-[rgba(255,255,255,0.15)] transition-all hover-lift" data-prefix="+31" data-country="NL">
                                <img src="https://flagcdn.com/24x18/nl.png" alt="Nederland" class="inline-block mr-2 w-6 h-4"> Nederland (+31)
                            </button>
                            <button class="prefix-option w-full text-left py-3 px-4 bg-[rgba(255,255,255,0.1)] rounded-lg hover:bg-[rgba(255,255,255,0.15)] transition-all hover-lift" data-prefix="+33" data-country="FR">
                                <img src="https://flagcdn.com/24x18/fr.png" alt="Frankrijk" class="inline-block mr-2 w-6 h-4"> Frankrijk (+33)
                            </button>
                            <!-- Voeg meer landen toe met hun vlaggen -->
                        </div>
                        <button id="close-phone-prefix-popup" class="mt-4 w-full bg-gray-600 text-white py-2 rounded-lg hover:bg-gray-700 transition-all hover-lift">Sluiten</button>
                    </div>
                </div>

                <!-- Date Popup -->
                <div id="date-popup" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center hidden z-50">
                    <div class="bg-dark rounded-xl p-6 w-full max-w-md shadow-xl">
                        <h3 class="text-lg font-bold mb-4 font-['Playfair_Display']">Kies je geboortedatum</h3>
                        <div class="grid grid-cols-3 gap-4">
                            <!-- Day Dropdown -->
                            <div class="relative">
                                <button type="button" id="day-btn" class="w-full bg-[rgba(255,255,255,0.1)] text-white border-none rounded-lg py-3 px-4 text-left focus:outline-none glow transition-all flex items-center justify-between hover:bg-[rgba(255,255,255,0.15)]">
                                    <span id="day-selected">Dag</span>
                                    <i class="fas fa-chevron-down"></i>
                                </button>
                                <div id="day-dropdown" class="absolute w-full bg-dark rounded-lg mt-1 max-h-48 overflow-y-auto hidden z-50 popup-content">
                                    <?php for ($i = 1; $i <= 31; $i++): ?>
                            ...
                            <!-- Month Dropdown -->
                            <div class="relative">
                                <button type="button" id="month-btn" class="w-full bg-[rgba(255,255,255,0.1)] text-white border-none rounded-lg py-3 px-4 text-left focus:outline-none glow transition-all flex items-center justify-between hover:bg-[rgba(255,255,255,0.15)]">
                                    <span id="month-selected">Maand</span>
                                    <i class="fas fa-chevron-down"></i>
                                </button>
                                <div id="month-dropdown" class="absolute w-full bg-dark rounded-lg mt-1 max-h-48 overflow-y-auto hidden z-50 popup-content">
                            ...
                            <!-- Year Dropdown -->
                            <div class="relative">
                                <button type="button" id="year-btn" class="w-full bg-[rgba(255,255,255,0.1)] text-white border-none rounded-lg py-3 px-4 text-left focus:outline-none glow transition-all flex items-center justify-between hover:bg-[rgba(255,255,255,0.15)]">
                                    <span id="year-selected">Jaar</span>
                                    <i class="fas fa-chevron-down"></i>
                                </button>
                                <div id="year-dropdown" class="absolute w-full bg-dark rounded-lg mt-1 max-h-48 overflow-y-auto hidden z-50 popup-content">
                        ...
                        <button id="save-date" class="mt-4 w-full bg-primary text-white py-2 rounded-lg hover:bg-[#a00d26] transition-all hover-lift">Opslaan</button>
                        <button id="close-date-popup" class="mt-2 w-full bg-gray-600 text-white py-2 rounded-lg hover:bg-gray-700 transition-all hover-lift">Sluiten</button>
                    </div>
                </div>

                <!-- Login Link -->
                <p class="text-center text-gray-300 mt-6 text-sm">Al een account? <a href="/Maqua/aanmelden/" class="text-primary hover:underline">Meld je hier aan</a></p>
            </div>
        </div>
    </section>

    <!-- Footer -->
    <footer class="bg-[#0A0A1F] pt-20 pb-10">
        <div class="container mx-auto px-4 sm:px-6 lg:px-8">
            <div class="grid grid-cols-1 md:grid-cols-4 gap-10 mb-16">
                ...
                <!-- Quick Links -->
                <div>
                    <h3 class="text-lg font-bold mb-6 relative inline-block font-['Playfair_Display']">Snelle Links<span class="absolute bottom-0 left-0 w-full h-0.5 bg-primary"></span></h3>
                    <ul class="space-y-3">
                ...
                <!-- Contact Info -->
                <div>
                    <h3 class="text-lg font-bold mb-6 relative inline-block font-['Playfair_Display']">Contact<span class="absolute bottom-0 left-0 w-full h-0.5 bg-primary"></span></h3>
                    <ul class="space-y-4">
                ...
                <!-- Newsletter -->
                <div>
                    <h3 class="text-lg font-bold mb-6 relative inline-block font-['Playfair_Display']">Blijf op de hoogte<span class="absolute bottom-0 left-0 w-full h-0.5 bg-primary"></span></h3>
                    <p class="text-gray-400 mb-4">Meld je aan voor onze nieuwsbrief.</p>
                    <form class="mb-4">
                        <div class="flex">
                            <input type="email" placeholder="Je e-mailadres" class="py-3 px-4 rounded-l-lg w-full bg-[rgba(255,255,255,0.1)] text-white border-none focus:outline-none glow">
                            <button type="submit" class="bg-primary text-white py-3 px-4 rounded-r-lg hover:bg-[#a00d26] transition-colors hover-lift"><i class="fas fa-paper-plane"></i></button>
                        </div>
                    </form>
                </div>
            </div>
    ...
    <!-- JavaScript for Form Steps, Password Toggle, and Mobile Menu -->
    <script>
